...
namespace MelhoresPraticasTp3.Domain.Rules.Conditionals {
    public class AndSpecification<TModel> : CompositeSpecification<TModel> {
        private readonly ISpecification<TModel> _leftRule;
        private readonly ISpecification<TModel> _rightRule;

        public AndSpecification(ISpecification<TModel> leftRule, ISpecification<TModel> rightRule) {
            _leftRule = leftRule;
